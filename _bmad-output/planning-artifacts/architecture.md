...
**`null`, not `0`, for "no data yet."** Directly implements the UX spec's FR38 decision that an em dash (not "0%") is shown when a segment has no completed sessions — a magnitude the display layer can distinguish from a real 0% without a separate boolean flag. [Review][Decision] found via Story 4.3's code review 2026-09-08: `prd.md`'s FR38 wording stated the opposite ("shows 0%"), contradicting both this document and `ux-design-specification.md`'s "Segment History Log Addition" section, which gives a specific rationale — on the summary line, "0%" would misread as "scored zero" rather than "never attempted," a misreading the sort control's internal 0%-as-comparison-value never surfaces to the user. Resolved in favor of architecture.md/ux-design-specification.md; `prd.md`'s FR38 text corrected to match. The sort function (below) is the one place that *does* want a numeric floor for empty segments, and re-maps `null → 0` locally there rather than baking that choice into the shared calculation.
