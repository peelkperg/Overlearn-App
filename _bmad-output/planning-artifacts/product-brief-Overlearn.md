...
**Note (2026-09-06) — the overlearning-level Settings screen was pulled forward and redesigned; this list is superseded on that one item.** It is now **v1.1** scope, specified in `prd.md` as FR35–FR37. The design also changed: instead of a 50%/100% toggle, the setting accepts any value from **50% to 300% in 10% increments**, applied globally and taking effect immediately — including for a session already in progress. `TARGET_FLOOR` remains fixed at 5 and is not user-configurable. Every other item in the list above stands as written. v1.0 shipped with the level fixed at 50% exactly as this brief specified, and is frozen at git tag `v1.0.0`.
