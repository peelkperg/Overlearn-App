...
### Short/memorable public URL (dedicated domain, e.g. overlearn.app)

**Priority:** Future — funding-gated, not tiered P1-P4. **Discussed:** 2026-09-16. `overlearn.villarrealsalinas.com` is long and hard to share with international users, especially spoken/typed from memory. A dedicated short domain (e.g. `overlearn.app`, ~$12-20/year) was the ideal fix, but explicitly rejected for now: **"for now is free and for educational purpose only, so no funds to finance a domain name at this time"** (Gerardo). Revisit if/when the app becomes commercial.

**Interim solution, live (2026-09-16):** `https://bit.ly/overlearn` — a free Bitly branded short link pointing at the Google Sites landing page, zero cost, no code/DNS change. Chosen over TinyURL for Bitly's better long-term reliability/uptime track record and free custom-back-half support. QR code for the same link saved at `assets/images/overlearn-share-qr.png`.
