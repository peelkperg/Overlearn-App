...
**Priority:** `P4`. **Requested:** 2026-09-16, by Gerardo, after asking about a subdomain redirect for `overlearn.villarrealsalinas.com`. Decided that day to solve it with GoDaddy subdomain forwarding (DNS/registrar-level, no code change) instead — this entry is the deferred code-side alternative, not the chosen approach.

Serve the app directly at the custom subdomain (e.g. `overlearn.villarrealsalinas.com`) as a real GitHub Pages custom domain, rather than redirecting to `peelkperg.github.io/Overlearn-App/`. Would remove the redirect hop and let an installed PWA carry the custom domain as its origin.
