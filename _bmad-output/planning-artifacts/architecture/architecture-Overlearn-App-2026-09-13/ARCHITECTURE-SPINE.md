...
- **Rule:** exactly one named config key is the source of truth: `app.json`'s `expo.extra.basePath`. The router config, the PWA manifest fields, and the service worker's registration (AD-8) all read this same key directly — none re-derives or hardcodes the path independently. Set to the GitHub Pages subpath (e.g. `/Overlearn-App/`) for now. Revisit as one coordinated change to this single key, not three separate ones, if a custom domain is attached later (value becomes `/`).
...
- **Rule:** registration lives in exactly one file (the web entry point / root layout's web-only boot path), called once. No explicit `scope` option is passed — it defaults to the service worker file's own location under the base path (AD-3), which is already correct by construction.
