...
- [x] [Review][Patch] **AC #1's "immediately to the right of the trigger" is asserted but never tested** — an implementation rendering the toggle above or left of the trigger passes every test in the file. Add an assertion on sibling order / row layout. [src/components/SortControl.test.tsx:84-91]
- [x] [Review][Patch] **AC #2's "and the list re-sorts live" has no coverage at any level** — the component test asserts only that `onChange` fires; nothing exercises the toggle through `handleSortChange` → `setSortOption` → re-render. The wiring is pre-existing and correct, so risk is low, but the AC's observable outcome is unverified. Add an integration test in `src/app-tests/index.test.tsx`. [src/components/SortControl.test.tsx:94-106]
- [x] [Review][Patch] **`architecture.md`'s FR42 snippet was never synced to the shipped label+hint split** — CLAUDE.md §13.4 requires the spec be corrected in the same or immediately following commit; `git show --stat 611584b` touched no planning artifact. Its Enforcement Guideline is also worded for "the new toggle's accessibility label" only. Sync it to whatever the decision above produces. [_bmad-output/planning-artifacts/architecture.md:784-791, 826]
- [x] [Review][Patch] **File List omits two files the commit actually changed** — `sprint-status.yaml` and the story file itself. Same omission Story 4.6's review found; worth fixing the habit, not just the instance. [this file]

Applied 2026-09-12. The resolved decision (single `accessibilityLabel`, no hint) also required updating the trigger's own label (drops its direction clause) and two pre-existing test files that asserted the old label text (`SortControl.test.tsx`'s Story 4.3 block, `src/app-tests/index.test.tsx`'s AC #6/#8 tests) — not a new defect, a direct consequence of the decision. `epics.md` and `ux-design-specification.md` synced alongside `architecture.md`, per CLAUDE.md §13.4. Full suite: 391/391 passing (388 + 3 new: sibling-order test, AC #3 desc-case regression test, toggle-triggers-live-resort integration test). `tsc --noEmit` clean. Lint: same pre-existing 1 error/1 warning baseline, in files this patch pass never touched.
...
**Modified:**
- `src/components/SortControl.tsx` (new toggle `Pressable`, `flip()`/`DirectionGlyph` helpers, `flippedDirection` computed once and reused for `onPress`/label, `styles.row`/`styles.directionToggle` gap + `flexShrink`, restructured return JSX to wrap trigger+toggle in a row `View`)
- `src/components/SortControl.test.tsx` (new `[Story 4.7]` describe block; code review patches: exact-text assertions, `getByLabelText`, sibling-order test, desc-case AC #3 regression test, menu-not-opened/call-count assertions; Story 4.3 block's two trigger-label tests updated for the dropped direction clause)
- `src/app-tests/index.test.tsx` (new integration test for the toggle driving a live re-sort; two pre-existing AC #6/#8 tests updated for the trigger label's dropped direction clause)
- `_bmad-output/planning-artifacts/architecture.md` (FR42 snippet and Enforcement Guideline synced to the single-label decision)
- `_bmad-output/planning-artifacts/ux-design-specification.md` (FR42 accessibility passage synced)
- `_bmad-output/planning-artifacts/epics.md` (AC #4 example text and UX-DR28 synced)
- `_bmad-output/implementation-artifacts/sprint-status.yaml` (4-7 status: review → done)
- `_bmad-output/implementation-artifacts/4-7-flip-sort-direction-via-toggle.md` (this file — patches checked off)
...
- 2026-09-12: Applied code review patches (11/11) and the resolved accessibility-strings decision. `flip()`/`DirectionGlyph` replace three independent ternaries; `flexShrink`/`numberOfLines`/`gap` fix the large-font-scale layout defect; toggle's accessibility label is now one string (key + current direction + resulting direction, no hint), and the trigger's label drops its now-redundant direction clause. Added/strengthened 8 test assertions across `SortControl.test.tsx` and `index.test.tsx`; synced `architecture.md`, `ux-design-specification.md`, `epics.md` per CLAUDE.md §13.4. 391/391 passing, `tsc` clean, lint unchanged from baseline. Status: review → done.
