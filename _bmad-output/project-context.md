...
- **`architecture.md`'s "Architectural Boundaries" section is stale on standard-screen scope (code review 2026-09-05):** it says `app/index.tsx` "talks only to `useSegments`/`useSegmentHistory`," but the same document's Requirements to Structure Mapping and its own prose elsewhere confirm `app/index.tsx` must also call `useActiveSession` — FR24's resume/discard prompt has to appear at app open, before the user navigates into a specific segment, and Home is the only screen that stays mounted for the app's whole lifetime to catch it. Not a code violation; the "Architectural Boundaries" wording just wasn't updated when the interruption-recovery design was finalized. Don't "fix" `index.tsx` to remove this import.
...
- **Naming (architecture.md, confirmed as-shipped):** components PascalCase (`CorrectButton.tsx`, one component per file, filename matches exactly); hooks camelCase with `use` prefix (`useActiveSession`, `useSegmentHistory`); functions camelCase verb-first (`calculateTargetStreak`, `writeHistoryEntry`). **Exception:** starter-template hooks *and components* kept their original kebab-case filenames (`use-color-scheme.ts`, `use-theme.ts`, `themed-text.tsx`, `themed-view.tsx`) — only Overlearn's own bespoke hooks/components follow the `UseXxx`/`PascalCase.tsx` convention; don't rename the inherited ones (20+ import sites) just for consistency. (Code review 2026-09-05: this exception previously only mentioned hooks in this file; extended to cover the equally-inherited component filenames, which were flagged as undocumented violations before this was found.)
- **Two undocumented naming/organization deviations found via code review 2026-09-05, deferred rather than fixed:** `src/components/web-badge.tsx` (exports `WebBadge`) and `src/components/animated-icon.tsx`/`.web.tsx` (the latter exports two components, `AnimatedSplashOverlay` and `AnimatedIcon`, from one file) are genuinely Overlearn-authored — not inherited starter files — so the exception above doesn't cover them. Splitting/renaming touches the splash-screen animation's import sites (`_layout.tsx`, `stack-screens.ts`) and needs a full re-test of that flow; treated as a deliberate follow-up, not a blind patch, given it's a pure organizational fix with no functional bug behind it.
